...
function validateContactForm(data: ContactFormData): Record<string, string> {
  ...
  if (
    data.phone &&
    data.phone.trim() &&
    !/^[+]?[0-9\s\-$$$$]{10,}$/.test(data.phone.replace(/\s/g, ""))
  ) {
...
export async function submitContactForm(
  ...
  try {
    ...
    const adminEmailResult = await resend.emails.send({
      from: "AnimaLife Contact <noreply@animalife.com>",
    ...
    const autoReplyResult = await resend.emails.send({
      from: "AnimaLife Support <support@animalife.com>",
      to: [data.email],
      subject: "Thank you for contacting AnimaLife!",
