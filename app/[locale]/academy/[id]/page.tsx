...
export async function generateStaticParams() {
  ...
  for (const post of AcademyPosts) {
    paths.push({ locale: 'en', id: post.slug });
  }
  
  for (const post of AcademyPostsAr) {
    paths.push({ locale: 'ar', id: post.slug });
  }
  
  return paths;
}

export async function generateMetadata({ params: { locale, id } }: { params: { locale: Locale; id: string } }) {
  const posts = locale === 'ar' ? AcademyPostsAr : AcademyPosts;
  ...
  return {
    ...
    keywords: locale === 'ar' 
      ? ['أنيمالايف', 'رعاية الحيوانات الأليفة', 'تغذية الكلاب', 'نصائح الحيوانات الأليفة', post.category]
      : ['AnimaLife', 'pet care', 'dog nutrition', 'pet tips', post.category],
    openGraph: {
      title: post.title,
      description: post.excerpt,
